...
public class XSDMapper {
	...
	public ObjectNode mapToInternalJson(String filePath) {
        ...
        if(root != null) {
            ...
            if(root.getXsdComplexType() != null) {
            	handleComplexType(root.getXsdComplexType(), props, null);
            }        	
        }
        else {
	...
	void handleElement(XsdElement e, ObjectNode props, ObjectNode newProps) {
		...
		if(elementName.equals("nameIdentifier")) {
		...
		if(e.getTypeAsBuiltInDataType() != null) {
			...
			if(e.getMaxOccurs() != null && e.getMaxOccurs().equals("unbounded")) {
				...
				if(e.getParent() instanceof XsdMultipleElements && ((XsdMultipleElements)e.getParent()).getXsdElements().count() == 1) {
					// array of these elements
					/*
					XsdElement arrayParent = findNextParentElement(e);
					if(arrayParent != null) {
						...
						newProps.set(arrayParent.getName(), newObj);
						newObj.put("type", "array");
						newObj.set("items", newProps2);
						
						newProps2.put("type", "object");
						newProps2.put("title", e.getName());
						newProps2.set("properties", newProps3);
						newProps3.set(e.getName(), prop);				
					}
					*/
					ObjectNode newObj = m.createObjectNode();
					ObjectNode newProps2 = m.createObjectNode();
			...
			// additional conditions - multiple element the e belongs to only has one element (this one)
			if(elementId != null) {
	...
	private void handleBuiltInType(XsdBuiltInDataType datatype, ObjectNode props) {
		...
		case "dateTime": {
			props.put("type", "string");
			props.put("pattern", "^([\\+-]?\\d{4}(?!\\d{2}\\b))((-?)((0[1-9]|1[0-2])(\\3([12]\\d|0[1-9]|3[01]))?|W([0-4]\\d|5[0-2])(-?[1-7])?|(00[1-9]|0[1-9]\\d|[12]\\d{2}|3([0-5]\\d|6[1-6])))(T((([01]\\d|2[0-3])((:?)[0-5]\\d)?|24\\:?00)([\\.,]\\d+(?!:))?)?(\\17[0-5]\\d([\\.,]\\d+)?)?([zZ]|([\\+-])([01]\\d|2[0-3]):?([0-5]\\d)?)?)?)?$");
			break;
		}
		case "date": {
			props.put("type", "string");
	...
	void handleComplexType(XsdComplexType e, ObjectNode props, ObjectNode newProps) {		
		...
		*/
		/*
		if(e.getChildAsSequence() != null) {
			XsdSequence seq = e.getChildAsSequence();*/
			/*
			if(seq.getMaxOccurs() != null) {
				newProps = handleMaxOccurs(seq.getMaxOccurs(), newProps);
			}
			*/
			/*
			if(e.getParent() instanceof XsdElement) {
				ObjectNode newObj = m.createObjectNode();
				ObjectNode newProps2 = m.createObjectNode();
				newObj.set("properties", newProps2);
	...
	ObjectNode handleSimpleType(XsdSimpleType e, ObjectNode props) {
		...
		if(e.getUnion() != null) {
			...
			if(r.getEnumeration() != null && r.getEnumeration().size() > 0) {
				ArrayNode a = m.createArrayNode();
				r.getEnumeration().forEach(en -> { a.add(en.getValue()); });
				prop.set("enum", a);
			}
		}
